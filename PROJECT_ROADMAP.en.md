...
- [ ] **Step 4: The Epistemic Trial (Dynamic Scoring):** Implement the "Debate Mode." Users can enter the context of a specific Node and debate Socrates. The user's goal is to introduce new logical parameters to convince the AI to update and alter the initial Logical Coherence Score of that node.
- [ ] **Step 5: Fractal Score Percolation (The Bubbling Algorithm):** Implement the backend math/triggers where the weight and score of a Child Node dynamically affect the Parent Node. If a "Challenge" edge has a higher logical score than the node it attacks, the parent's score is reduced recursively up to the Macro-Arena.
