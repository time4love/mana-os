...
- [ ] **Step 4: The Crossfire UI (Categorized Debate):** Redesign the Claim Detail view. Instead of a health bar, display the claim's **Epistemic State** (`SOLID`, `CONTESTED`, `SHATTERED`). Below it, display attackers and defenders **grouped by their Epistemic Move Category** (e.g., a red box showing "Attacked by: Empirical Contradiction", a yellow box showing "Defended by: Ad-Hoc Rescue"). Tap-to-Focus: tapping a claim reveals only the claims that attack or defend it; never rely on CSS `:hover` for critical epistemic relationships.
