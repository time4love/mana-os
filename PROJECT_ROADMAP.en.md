...
**Ordering principle:** The **Personal Sense-Making MVP** (single-user ingest → structured Arena → **Cartographer** guidance) is the near-term product surface. Full **multi-player** Truth Engine features—**Genesis Anchor SBT** gates, **Epistemic Resonance** as tribunal, and **on-chain** hashing—follow once the solo map-and-gaps loop is stable. Implement accordingly: do not block sense-making UX on Web3 completion, but keep APIs shaped for later Sybil resistance.

- [x] **Step 1: The DAG Architecture:** Create `truth_nodes` and `truth_edges` in Supabase allowing concepts to have multiple parents (Directed Acyclic Graph). (Completed.)
- [x] **Step 2: Macro-Arenas & Topic Initialization:** Build the UI for establishing large-scale discussion arenas (e.g., "Globe vs. Flat Earth"). Root Nodes as open questions with competing theories (Theory A vs. Theory B). (Completed.)
- [x] **Epistemic state migration:** Replace `logical_coherence_score` with `epistemic_state` enum (`SOLID`, `CONTESTED`, `SHATTERED`); claims transition by community refutation. (Completed.)
- [ ] **Step 3: The Universal Sieve (Bulk Ingestion):** Build the Sieve to extract claims from transcripts, but update the **Logician Agent** to classify claims into specific **Epistemic Move Categories** (Empirical Contradiction, Internal Inconsistency, Empirical Verification, Ad-Hoc Rescue, Appeal to Authority) rather than assigning numeric scores.
- [ ] **Step 4: The Crossfire UI (Categorized Debate):** Redesign the Claim Detail view. Instead of a health bar, display the claim's **Epistemic State** (`SOLID`, `CONTESTED`, `SHATTERED`). Below it, show **only the claim and its attackers**: child edges in the DAG are **challenges**—there are no "Defended by" or "Supporting children" columns (bolstering happens via **Sharpening**, not extra nodes). Group **attackers** by **Epistemic Move Category** where helpful (e.g. "Challenged by: Empirical Contradiction"). **Tap-to-Focus:** tapping a claim reveals only the claims it attacks or that attack it; never rely on CSS `:hover` for critical epistemic relationships.
- [ ] **Step 4b: The Sharpening Drawer & Version History:** Build the UI for users to **Sharpen** an existing claim—supplying new user-owned data or logic that the **Blacksmith** persona merges into the next immutable version (e.g. V1.0 → V1.1). Implement a carousel, stepper, or dropdown so readers can traverse **prior versions** of the same claim and see how the community forged it over time. The AI acts only as **Logical Blacksmith**: it forges upgraded text from human input; it does not invent evidence.
- [ ] **Step 4c: The Cartographer Chat (Sense-Making MVP):** Add a dedicated **read-only, analytical** chat in the Arena lobby (**"Ask the Weave"** or equivalent). The **Cartographer** persona summarizes the debate map, compares theory columns against **anchored** structure only, and lists **Epistemic Gaps** (unanswered challenges, missing empirical hooks) to steer the user's **external** research. No arguing, no scoring, no inventing evidence.
- [ ] **Step 4d: Cognitive Caching Integration:** Implement backend support so the Cartographer does **not** re-send the full Arena DAG inside every chat `messages` payload. Flatten the Macro-Arena (nodes, edges, epistemic states, theory metadata) into a stable **text/JSON document**, register it with a **context caching** API (e.g. **Google Gemini Context Caching** or an equivalent pattern), and run multi-turn follow-ups against that cached context so latency and token use stay bounded as arenas grow.
- [ ] **Step 5: Epistemic Resonance (Community Tribunal):** The community votes on the validity of the *Epistemic Move* itself. If the community resonates heavily with an Empirical Contradiction, it shatters the parent claim. State transitions (Solid → Contested → Shattered) are driven by Epistemic Resonance on refutations, not numeric multipliers.
- [ ] **Step 6: The Universal Sieve (Global Input):** Deprecate individual claim-level input drawers. Create one global **Feed the Arena** action. The AI ingests a transcript, extracts claims, classifies them into Epistemic Move Categories, aligns them to a Theory, and *cross-matches* them to find if they attack existing claims in the opposing Theory. Single entry point for feeding the Crossfire Arena.
...
- [ ] **Step 15: The Epistemic Crucible (Experiment Nodes):** Introduce **`node_type: 'EXPERIMENT'`** (or equivalent) with **first-class schema** for **`methodology`**, **`observed_results`** (nullable until a run finishes), and **`epistemic_impact`** (typed links / move classification vs target claims). Build **two distinct flows**:
  1. **The Lab Drawer (Historical ingestion):** Dedicated UI (**“Submit Experiment Report”**) for **completed** experiments from the literature or field—users paste or upload reports. Pipeline uses the **Lab Auditor** persona: structured extraction only (methodology, variables, observed results); **no fabricated measurements**; output explicitly ties to an **Epistemic Move** (e.g. **Empirical Contradiction** / **Empirical Verification**) and to **which claims** are refuted or supported. Lifecycle may use states such as **`INGESTED`** / **`VERIFIED`** (community-anchored) distinct from proposed-work states.
  2. **The Proposal Forge (Future execution):** Center **Crucible** strip in the War Room for **Proposed** protocols—**Scientific Auditor** hardens falsifiability; lifecycle **`PROPOSED` → `ACTIVE`** (linked to **Governance / DAO** and **Mana Callings**) **→ `CONCLUDED`** (observed results written; Epistemic Moves generated from **new** empirical data, feeding claim state transitions). **Must not** forecast outcomes while `PROPOSED`.
