...
### 🔮 Phase 10: The Fractal Truth Engine (The Epistemic Arena)
...
- [ ] **Step 2: Macro-Arenas & Topic Initialization:** Build the UI for establishing large-scale discussion arenas (e.g., "Globe vs. Flat Earth"). These act as Root Nodes that anchor thousands of sub-claims.
- [ ] **Step 3: The Transcript Sieve (Bulk Ingestion):** Build a dedicated pipeline for ingesting long transcripts (e.g., a 20-minute video transcript). The Sieve Agent extracts distinct logical claims, scores them, and structurally maps them: identifying which claims directly support the Macro-Arena, and which claims attack *other specific sub-claims* within the weave.
- [ ] **Step 4: The Mini-Arena UX (Claim-Level Debate):** Transform the standard Node Detail page into a tactical debate interface. Split the actions into explicit "Support this Claim" (בסס) and "Challenge this Claim" (הפרך). Render child nodes in two distinct columns: Supporters vs. Challengers.
- [ ] **Step 5: The Bubbling Algorithm Math:** Implement dynamic graph calculations. A node's **Current Validity** = **Base Score** + (Sum of Supporting Children) − (Sum of Challenging Children). Ensure Current Validity cannot drop below 0 or exceed 100. Map this dynamic weight directly to the Macro-Arena's balance scales.
- [ ] **Step 6: Endless Dive UX:** Implement a sliding, horizontal Miller-Column layout that isolates singular logic junctions, preventing visual collapse when diving into deep, multi-layered sub-debates.
...
- [ ] **Step 9: On-Chain Hashing & Consensus:** Create the **EpistemicWeave.sol** smart contract. Implement a mechanism where finalized node scores and their content/IPFS hashes are anchored on-chain via gasless transactions (ERC-4337) after **Anonymous Community Resonance**. The Immutable Epistemic Ledger is born.
- [ ] **Step 10: Epistemic Forking Protocol:** Build the UI and backend logic allowing a community to **fork** a specific Truth Node branch when they reject the AI's logical evaluation. Both the original and the forked path persist on-chain; users choose which logical reality to explore. No censorship—only branching.
- [ ] **Step 11: Epistemic Resonance & The Multiplier (Blockchain Integration):** Build the UI mechanism on the Node Detail page allowing verified SBT holders to cast an "Epistemic Resonance" vote on a claim. Update the Bubbling Algorithm formula so that `Node Mass = (Base AI Score) × (Community Resonance Multiplier)`. This is the ultimate defense against AI bias.
