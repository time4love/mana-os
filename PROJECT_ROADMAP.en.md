...
- [ ] **Step 3: The Socratic Forge & Human-in-the-Loop Drafting:** The AI acts strictly as a conversational partner, breaking down large texts into Socratic dialogue. It NEVER auto-generates Draft Cards for multiple claims.
- [ ] **Step 4: The Single-Seed Protocol (User-Driven Execution):** The user must explicitly command the AI: "Please add this specific claim to the weave: [text]". ONLY THEN does the AI run quality checks, vector deduplication, and generate EXACTLY ONE Draft Card.
- [ ] **Step 5: Anti-Bulk Injection Guardrail:** If a user asks to draft a multi-claim paragraph, the AI refuses to call the tool. Instead, it outputs a text list of the separated claims and asks the user to submit them one by one for focused, mindful evaluation.
- [ ] **Step 6: Endless Dive UX & Epistemic Resonance:** Implement a sliding, horizontal column layout that isolates singular logic junctions, preventing visual collapse and infinite nesting scroll-fatigue. Implement anti-troll voting (`Verify Logic` / `Flag Fallacy`). Only users holding Genesis Anchor SBTs can affect the structural weight of a node.
- [x] **Universal Vector Translation (The Rosetta Node):** Data nodes embed solely off underlying conceptual **Universal English** equivalents. The Forge produces bilingual drafts (English + Hebrew) at the drafting stage; embeddings and semantic matching use only the English assertion so that vector distance and clustering are identical regardless of input language. The UI displays localized content (he/en) at runtime, seamlessly crossing multi-continental barriers inside an endless DAG.
