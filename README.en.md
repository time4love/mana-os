...
Claims exist in one of three **Epistemic States**: **Solid (יציבה)**—no community-validated refutations; **Contested (מעורערת)**—under direct attack from the opposing theory; **Shattered (הופרכה)**—the community has validated the refutation (Shattered claims remain in the weave as educational scars). Debates happen in the **Crossfire Arena**: two theories (e.g. Theory A vs. Theory B) clash horizontally. A claim in Theory A can directly attack a specific claim in Theory B; the community **Resonates** on counter-arguments, and when consensus validates a refutation, the targeted claim transitions to Shattered. The AI acts as a blind, objective referee of formal logic; it never invents evidence. One global **Feed the Arena** flow lets users submit transcripts—the AI extracts claims, aligns them to a theory, and cross-matches them against the opposing theory.
